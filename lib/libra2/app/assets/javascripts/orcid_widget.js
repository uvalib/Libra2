    ...
    function initPage() {
        ...
        $.fn.orcid_widget = function (options, param) {
            ...
            var defaults = {
                ...
                search_endpoint: 'https://pub.orcid.org/v1.2/search/orcid-bio?q=',
                lookup_endpoint: 'https://pub.orcid.org/v1.2/',
                ...
                clear_class: 'orcid_clear btn btn-default btn-small',
                clear_text: 'Clear',
                search_text: 'Search',
        ...
        function bind_orcid_plugin(obj, settings) {
            ...
            if (settings.search) {
                ...
                var clear_btn = $('<button>').addClass(settings.clear_class).html(settings.clear_text);
                var search_btn = $('<button>').addClass(settings.search_class).html(settings.search_text);
                var search_html = settings.query_text + ' <input type="text" class="orcid_search_input"/> <button class="btn btn-default search_orcid">' + settings.search_text_btn + '</button><div class="orcid_search_result"></div><a class="close_search">' + settings.close_search_text_btn + '</a>';
                var search_div = $('<div>').addClass(settings.search_div_class).html(search_html);

                if (!settings.pre_open_search) $(search_div).hide();
                p.append( search_btn ).append( clear_btn ).append( search_div );
                ...
                $('input.orcid_search_input', p).on('keypress', function (e) {
                    //enter key
                    if (e.keyCode == 13) {
                        _search($(this).val(), obj, settings);
                        return false;
                    }
                });
                $('.orcid_clear', p).on('click', function (e) {
                    //click orcid clear
                    e.preventDefault();
                    e.stopPropagation();
                    $( '.orcid_search_result').empty( );
                    $( '.orcid_success_div').remove( );
                    obj.val( '');
                });
        ...
        function _constructORCIDHTML(obj, settings) {
            ...
            if (obj['orcid-bio']['keywords']) {
                ...
                var count = 0;
                for (var i = 0; i < (obj['orcid-bio']['keywords']['keyword'].length - 1); i++) {
            ...
            if (obj['orcid-bio']['researcher-urls']) {
                ...
                var count = 0;
                for (var i = 0; i < (obj['orcid-bio']['researcher-urls']['researcher-url'].length - 1); i++) {
        ...
        function _search(query, obj, settings) {
            ...
            if ($.trim(query) == "") {
                ...
                $.ajax({
                    url: settings.search_endpoint + encodeURIComponent(query) + '&start=0&rows=25',
                    //dataType: 'json',
                    contentType: 'application/json',
                    success: function (data) {
                        if (settings.success_handler && (typeof settings.success_handler === 'function')) {
                            ...
                            if (data['orcid-search-results']) {
                                ...
                                $.each(data['orcid-search-results']['orcid-search-result'], function () {
                                    ...
                                    var given = '';
                                    if( this['orcid-profile']['orcid-bio']['personal-details']['given-names'] ) {
                                        given = this['orcid-profile']['orcid-bio']['personal-details']['given-names'].value
                                    }

                                    var family = '';
                                    if( this['orcid-profile']['orcid-bio']['personal-details']['family-name'] ) {
                                        family = this['orcid-profile']['orcid-bio']['personal-details']['family-name'].value
                                    }
                                    html += '<li>';
