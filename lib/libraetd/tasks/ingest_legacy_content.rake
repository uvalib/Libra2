...
# pull in the helpers
require_dependency 'libraetd/tasks/ingest_helpers'
...
namespace :libraetd do
  ...
  # ingest content
  #
  desc "Remove legacy Libra content; must provide the ingest directory; optionally provide the start index"
  task remove_legacy_content: :environment do |t, args|
    ...
    depositor = Helpers::EtdHelper::lookup_user( IngestHelpers::DEFAULT_DEPOSITOR )
    if depositor.nil?
      puts "ERROR: Cannot locate default user info (#{IngestHelpers::DEFAULT_DEPOSITOR})"
      next
    end

    user = User.find_by_email( depositor.email )
    if user.nil?
      puts "ERROR: Cannot lookup default user info (#{depositor.email})"
      next
    end
    ...
    ingests.each_with_index do | dirname, ix |
      next if ix < start_ix
      ok = remove_legacy_content(  user,File.join( ingest_dir, dirname ), ix + 1, total )
      ok == true ? success_count += 1 : error_count += 1
      break if ENV[ 'MAX_COUNT' ] && ENV[ 'MAX_COUNT' ].to_i == ( success_count + error_count )
    end
  ...
  #
  # remove legacy content to an existing metadata record
  #
  def remove_legacy_content( user, dirname, current, total )

    puts "Removing #{current} of #{total}: #{File.basename( dirname )}"
